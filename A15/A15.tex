...
\begin{figure}[!ht]
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \begin{scope}[shift={(-4,0)}]
            ...
            \draw[thick] (0,0,0) -- (1,1,0) -- (0,2,0) -- (-1,1,0) -- cycle;
            \draw[thick] (0,0,0) -- (1,-1,0) -- (0,-2,0) -- (-1,-1,0) -- cycle;
            \draw[thick] (0,0,0) -- (0,0,2) -- (1,1,2) -- (0,2,2) -- (-1,1,2) -- cycle;
            \draw[thick] (0,0,0) -- (0,0,-2) -- (1,-1,-2) -- (0,-2,-2) -- (-1,-1,-2) -- cycle;
...
\begin{figure}[!ht]
    ...
    \begin{tikzpicture}[
        node distance=1.5cm,
        box/.style={rectangle, draw, text width=2.5cm, rounded corners, minimum height=1cm, text centered},
        arrow/.style={->, >=stealth, thick},
        process/.style={rectangle, draw, fill=blue!10, text width=2.5cm, rounded corners, minimum height=1cm, text centered},
        data/.style={rectangle, draw, fill=green!10, text width=2.5cm, rounded corners, minimum height=1cm, text centered},
        method/.style={rectangle, draw, fill=red!10, text width=2.5cm, rounded corners, minimum height=1cm, text centered},
        stability/.style={rectangle, draw, fill=yellow!10, text width=2.5cm, rounded corners, minimum height=1cm, text centered}
    ]

    % Main pipeline
    \node[data] (input) {Continuous 3D Coordinates (Floats)};
    \node[process, right=of input] (quantize) {Quantization Process};
    \node[data, right=of quantize] (identifier) {Compact A15 Identifier (Integer)};
    \node[process, right=of identifier] (reconstruct) {Deterministic Reconstruction};
    \node[data, right=of reconstruct] (output) {Consistent 3D Coordinates (Floats)};

    % Connections
    \draw[arrow] (input) -- (quantize);
    \draw[arrow] (quantize) -- (identifier);
    \draw[arrow] (identifier) -- (reconstruct);
    ...
    \draw[arrow] (wph) -- (quantize);
    \draw[arrow] (tsp) -- (quantize);
    ...
    \draw[arrow] (binary) -- (reconstruct);
    \draw[arrow] (stable) -- (binary);
    \draw[arrow] (unstable) -- (reconstruct);

    % Internal Integer Representation
    \node[data, below=2cm of identifier] (internal) {Internal Integer Representation\\(96-unit baseline)};
    \draw[arrow, dashed] (internal) -- (quantize);
    \draw[arrow, dashed] (internal) -- (identifier);
    \draw[arrow, dashed] (internal) -- (reconstruct);

    % Network/Storage
    \draw[arrow, dotted, bend left] (identifier) to node[above] {Network/Storage} (reconstruct);

    % Environment boundaries
    \draw[rounded corners, dashed] (-1,-4) rectangle (3.5,4) {};
    \draw[rounded corners, dashed] (4,-4) rectangle (6,4) {};
    \draw[rounded corners, dashed] (6.5,-4) rectangle (11,4) {};

    \node at (1.25,-3.5) {Client Environment};
    \node at (5,-3.5) {Network/Storage};
    \node at (8.75,-3.5) {Server/Recipient Environment};
...
This glossary provides a reference for the specialized terms and mathematical notation used throughout the manuscript. For implementation details and additional technical specifications, refer to the \texttt{A15.py} reference implementation (\Cref{subsec-implementation-a15py}) and the supplementary information (\Cref{sec-supplementary}).
