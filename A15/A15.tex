...
\begin{figure}[!ht]
    ...
    \node[stability, below=of reconstruct] (unstable) {Unstable Regime\\$\epsilon_\Delta \neq 0$}
        edge [->, in=110, out=70, looseness=3] (unstable);
...
\begin{figure}[!ht]
    ...
    \begin{tikzpicture}[
        ...
        box/.style={rectangle, draw, text width=7cm, rounded corners, minimum height=1cm, text centered},
        arrow/.style={->, >=stealth, thick},
        regime/.style={rectangle, draw, text width=3.5cm, rounded corners, minimum height=2.5cm, align=center},
        hist/.style={rectangle, draw, minimum width=3.5cm, minimum height=1.5cm}
    ]

    % Title
    \node[font=\large\bfseries] (title) {A15 Stability Regimes};

    % Epsilon delta node
    \node[box, fill=blue!10, below=of title] (epsilon) {Output Scale Factor $\epsilon_\delta$};

    % Regimes
    \node[regime, fill=green!10, below=of epsilon] (binary) {
        \textbf{Binary}\\
        $\epsilon_\delta = \epsilon_N$\\
        $\epsilon_\Delta = 0, m = 1$\\[0.3cm]
        \textit{Perfect 1:1 mapping}\\
        \textit{Most compact scale}\\
        \textit{Strong determinism}
    } edge[<-, thick] (epsilon);

    \node[regime, fill=yellow!10, left=of binary] (stable) {
        \textbf{Stable}\\
        $\epsilon_\delta = m \cdot \epsilon_N, m > 1$\\
        $\epsilon_\Delta = 0$\\[0.3cm]
        \textit{Simple integer mapping}\\
        \textit{Linear multiple scale}\\
        \textit{Strong determinism}
    } edge[<-, thick, bend left=35] (epsilon);

    \node[regime, fill=red!10, right=of binary] (unstable) {
        \textbf{Unstable}\\
        $\epsilon_\delta \neq m \cdot \epsilon_N$\\
        $\epsilon_\Delta \neq 0$\\[0.3cm]
    ...
    } edge[<-, thick, bend right=35] (epsilon);

    % Histogram examples
    \node[hist, below=of binary] (histb) {}
        edge[<-, thick, dashed] (binary);
    \node[hist, below=of stable] (hists) {}
        edge[<-, thick, dashed] (stable);
    \node[hist, below=of unstable] (histu) {}
        edge[<-, thick, dashed] (unstable);

    % Binary histogram - minimal discrete peaks
    % TODO
    \node[font=\tiny, below=of histb, yshift=0.75cm] {Exponent $n = \log_2 d$};
    \node[font=\scriptsize, below=of histb, yshift=0.5cm] {Minimal Contiguous Discrete};

    % Stable histogram - multiple discrete peaks
    % TODO
    \node[font=\tiny, below=of hists, yshift=0.75cm] {Exponent $n = \log_2 d$};
    \node[font=\scriptsize, below=of hists, yshift=0.5cm] {Multiple Contiguous Discrete};

    % Unstable histogram - gapped or sparse distribution
    % TODO
    \node[font=\tiny, below=of histu, yshift=0.75cm] {Exponent $n = \log_2 d$};
    \node[font=\scriptsize, below=of histu, yshift=0.5cm] {Gapped or Sparse Distribution};

    % Formula
    \node[box, fill=gray!10, below=of histb, yshift=-0.5cm] {$\epsilon_\Delta = 0 \iff \epsilon_\delta = m \cdot \epsilon_N$ for $m \in \mathbb{Z}^+$};
